...
def fetch_weather(settings: Settings) -> pd.DataFrame:
    ...
    if response.status_code != 200:
        raise RuntimeError(
            f"Weather API request failed with status {response.status_code}: {response.text[:200]}"
        )

    payload = response.json()
    timestamp = datetime.utcnow().strftime("%Y%m%dT%H%M%SZ")
    raw_file = settings.raw_dir / f"weather_{timestamp}.json"
    _write_raw_payload(payload, raw_file)

    hourly = payload.get("hourly")
    if not hourly:
        raise RuntimeError("Weather payload missing 'hourly' section")

    df = pd.DataFrame(hourly)
    if df.empty:
        raise RuntimeError("Weather payload returned no hourly data")
